...
draw_sprite_ext(playerCardBackground, 0, x, y, image_xscale, image_yscale, 0, c_white, image_alpha);

draw_sprite_ext(playerPodiumSprite, 0, x, y, sc, sc, 0, c_white, 1);

playerDrawSimple(x, y - 36 * sc, sc * 1.2);
...
if (!is_undefined(username)) {
	...
	draw_text_transformed(x + 12, y - 105 * sc, username, 1, 1, 0);
	
	//draw icon
	draw_sprite_ext(rankIconSprite, rankTier, x - string_width(username)/2 - 8, y - 105 * sc, .5, .5, 0, c_white, 1);
	...
	draw_sprite_ext(skillTierIconSprite, 0, x - _w / 2 + _ss/2, y - 90 * sc, _ic, _ic, 0, c_white, 1)
	draw_text_transformed(x - _w / 2 + _ss + _s, y - 90 * sc, skillTier, .65, .65, 0);
	
	draw_sprite_ext(rankTierIconSprite, 0, x - _w / 2 + _ss + _s + _wst + _s + _ss/2, y - 90 * sc, _ic, _ic, 0, c_white, 1)
	draw_text_transformed(x - _w / 2 + _ss + _s + _wst + _s + _ss + _s, y - 90 * sc, rankName, .65, .65, 0);
	
	
	draw_sprite_ext(defaultIconBorder, 0, x, y - 138 * sc, 1.2, 1.2, 0, c_white, 1);
	draw_sprite_ext(IconList[iconIndex], 0, x, y - 138 * sc, 1.2, 1.2, 0, c_white, 1);
