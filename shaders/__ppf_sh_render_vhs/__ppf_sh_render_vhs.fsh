...
vec4 vhs_fx(vec2 uv, vec2 fpos) { // N APLICAR DISTORÇÃO NA IMAGEM. SÓ NOS EFEITOS
	vec4 _col;
	...
	// wiggle
	uv.y += sin(time*15.0 + sin(time*15.0)) * u_vhs_wiggle_amplitude;
	...
	float scan_noise = gold_noise(vec2(time, fpos.y), 1.0);
	...
	float chroma_dist = v_TexelSize.x * u_vhs_chromatic_aberration + scan_aberration_sum;
	...
	_col.rgb = mix(_col.rgb, vec3(scanlines), scanlines);
