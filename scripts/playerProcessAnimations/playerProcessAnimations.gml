...
run = dtlerp(run,clamp(movvec.length()/4,0,1),0.2)
...
wall_blend = dtlerp(wall_blend, state == PlayerState.WALL_SLIDING, 0.3);

jump_fast_prog = dtlerp(jump_fast_prog,clamp(abs(movvec.x/15),0,1),0.08)

grapple_blend = dtlerp(grapple_blend, state == PlayerState.GRAPPLED ,0.2);

grounded_blend = dtlerp(grounded_blend, state == PlayerState.GROUNDED, 0.2)

grounded = max(grounded - 1*dtime,0);

pushed_blend = dtlerp(pushed_blend, movvec.x*dir<-1.5, 0.3);

grapple_throw_blend = dtlerp(grapple_throw_blend, state == PlayerState.GRAPPLE_THROW, 0.4)
...
dash_blend = dtlerp(dash_blend, state == PlayerState.DASHING, 0.2);
